...
Public Class clsFilterData
                ...
                Try
                    ...
                    For Each entry As DBDictionaryEntry In dbDict
                        ...
                        For Each tv As TypedValue In datas
                            ...
                            End If
                        Next
                        Dim currFilt As String = entry.Key
            ...
            Using acTrans As Transaction = acCurdDb.TransactionManager.StartTransaction()
                ...
                Try
                    ...
                    For Each entry As DBDictionaryEntry In dbDict
                        ...
                        If filterName = entry.Key Then
                            result.Add(entry.Key, strs)
                            'MsgBox("filter gvonden " & String.Join(", ", strs))
                            Exit For
                        End If
                    Next
    ...
    Public Shared Function loadSettings(ByRef acDoc As Document, ByRef acCurrDb As Database, ByRef acEd As Editor, ByVal dictName As String, ByVal sSettingName As String)
        Dim aLoadFilters As List(Of String) = New List(Of String)
        Try
            Dim dict As Dictionary(Of String, List(Of String)) = clsFilterData.getFilterFromDictionary(acDoc, acCurrDb, acEd, dictName, sSettingName)
            For Each pair As KeyValuePair(Of String, List(Of String)) In dict
                For Each s As String In pair.Value
                    aLoadFilters.Add(s)
                Next
